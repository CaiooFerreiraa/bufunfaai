...
func Register(publicGroup *gin.RouterGroup, protectedGroup *gin.RouterGroup, handler *ofhandler.Handler) {
	...
	openFinanceGroup.GET("/accounts", handler.ListAccounts)
	openFinanceGroup.GET("/overview", handler.Overview)
	openFinanceGroup.GET("/transactions", handler.ListTransactions)
